// src/actions/createAndRegisterAgent.ts
...
var isAgentSettings = (value) => {
  return AgentSettingsSchema.safeParse(value).success;
};
var VerifyParamsSchema = z.object({
  agent: z.string(),
  digest: z.string(),
  payload: z.object({
    data: z.string(),
...
var isVerifyParams = (value) => {
  return VerifyParamsSchema.safeParse(value).success;
};
var PriceQueryParamsSchema = z.object({
  pair: z.string()
});
var AttpsPriceQuerySchema = z.object({
  sourceAgentId: z.string(),
  feedId: z.string()
});
var isAttpsPriceQuery = (value) => {
  return AttpsPriceQuerySchema.safeParse(value).success;
};

// src/actions/createAndRegisterAgent.ts
var createAndRegisterAgent = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      agentSettings = agentSettingsDetail.object;
    ...
    try {
      ...
      if (callback) {
        const message2 = (tx == null ? void 0 : tx.hash) ? `Error creating agent: ${errorMessage}. Transaction hash: ${tx.hash}` : `Error creating agent: ${errorMessage}`;
...
// src/actions/verifyData.ts
import { composeContext as composeContext2, elizaLogger as elizaLogger2, generateObject as generateObject2, ModelClass as ModelClass2 } from "@elizaos/core";
import { AgentSDK as AgentSDK2 } from "ai-agent-sdk-js";
var verifyData = {
  name: "VERIFY",
  similes: [
    "VERIFY_DATA"
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    let verifyParams;
    try {
      const response = await generateObject2({
        runtime,
        context: composeContext2({
          state: currentState,
      ...
      verifyParams = response.object;
      elizaLogger2.info("The verify params received:", verifyParams);
    } catch (error) {
      if (error instanceof Error) {
        elizaLogger2.error("Failed to generate verify params:", error.message);
      } else {
        elizaLogger2.error("Failed to generate verify params:", String(error));
      }
      callback({
    ...
    if (!isVerifyParams(verifyParams)) {
      elizaLogger2.error("Invalid verify params:", verifyParams);
      callback({
    ...
    try {
      ...
      if (error instanceof Error) {
        elizaLogger2.error("Failed to create Agent SDK:", error.message);
      } else {
        elizaLogger2.error("Failed to create Agent SDK:", String(error));
      }
      callback({
        text: "Failed to create Agent SDK. Please check the apro plugin configuration."
      });
    ...
    try {
      ...
      elizaLogger2.info("Data verification transaction sent. Transaction ID:", tx.hash);
      const receipt = await tx.wait();
      elizaLogger2.info("Data verification transaction confirmed. Transaction ID:", receipt.hash);
      callback({
        text: `Success: Data verified successfully. Transaction ID: ${receipt.hash}`
      });
    } catch (error) {
      if (error instanceof Error) {
        elizaLogger2.error(`Error verify data: ${error.message}`);
        let message2 = `Error verifying data: ${error.message}`;
        if (tx == null ? void 0 : tx.hash) {
          message2 = `${message2} Transaction hash: ${tx.hash}`;
        }
        callback({
          text: message2
        });
...
// src/actions/attpsPriceQuery.ts
import { composeContext as composeContext3, elizaLogger as elizaLogger3, generateObject as generateObject3, ModelClass as ModelClass3 } from "@elizaos/core";
async function fetchPriceData(sourceAgentId, feedId) {
...
var attpsPriceQuery = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      attpsPriceQuery2 = response.object;
      elizaLogger3.info("The price query params received:", attpsPriceQuery2);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      elizaLogger3.error("Failed to generate price query params:", errorMessage);
      if (callback) {
        callback({
    ...
    if (!isAttpsPriceQuery(attpsPriceQuery2)) {
      elizaLogger3.error("Invalid price query params:", attpsPriceQuery2);
      if (callback) {
        callback({
    ...
    try {
      const { sourceAgentId, feedId } = attpsPriceQuery2;
      const priceData = await fetchPriceData(sourceAgentId, feedId);
      elizaLogger3.info("The Price data received:", priceData);
      const message2 = `Ask price: ${cleanNumber(priceData.askPrice)}
...
${message2}`
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      elizaLogger3.error("Error fetching price data:", errorMessage);
      if (callback) {
        callback({
